...
### Master Node组件
- kube-apiserver
提供Kubenetes项目的操作接口，即kubenetes控制面板的前端
- kube-controller-manager
控制器管理组件，一般来说，一种类型的controller应该有自己单独的进程， 但为了减少复杂性，统一用单一进程管理。
controller类型包含：
...
- etcd
一致性和高可靠性键值存储设备，需要考虑备份恢复
- kube-scheduler
监测出没有被分配node的pod状态，并考虑资源需求(affinity specification)来分配node运行pod

### Worker Node组件
- kubelete
根据podspec文件保证pod中每个container能正常按需运行
- kube-proxy
网络代理
- container runtime
用于运行container的，比如docker/containerd/cri-o

### Addons
用k8s的资源（deployment/da）
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3MzIxMTk0ODMsMTE5OTc2NTEwLC00MT
gwNzk0OTgsMTYzMTg3Mjc4Myw3ODU3MzY0MjcsLTE2NjQ4Mjg5
MDYsMTcyMzQ3NDQzOCwtMTA0OTM2MjE1OSw5NjQxMzM1MjMsLT
cwMDM5MjMzMiwtMTc3ODUzOTQzNywtNTQ1ODUwMTgsLTE3MzQ2
NzE3NTksMTk5MzMxMjA3NSwtMTc3MjcyMDYsLTUzMDk3NzU3MC
wtMTkxNzkzMjY0MywxNjEyNDAzMDIsLTEzMzMzMzYwOTYsMTA4
NDc4NTAzMl19
-->